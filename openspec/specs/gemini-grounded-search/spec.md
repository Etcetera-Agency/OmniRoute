# gemini-grounded-search Specification

## Purpose
Define Gemini native Google Search grounding as a `/v1/search` provider that
reuses existing Gemini credentials, returns the standard OmniRoute search
response shape, and acts as the final automatic web-search fallback.
