...
- [x] `src/lib/fmoPools/quota.ts` — replace the `extractQuotaClaimWithInternalLlm` stub
...
- [x] Confirm `response_format` reaches both the Claude and Gemini translators
      unchanged; do not assume provider-native JSON mode.
- [x] Tests (`tests/unit/fmo-pools-quota-extractor.test.ts`): well-formed JSON ⇒ parsed
...
- [x] Update `add-fmo-pools-planning` follow-through note (or this change's archive) so
