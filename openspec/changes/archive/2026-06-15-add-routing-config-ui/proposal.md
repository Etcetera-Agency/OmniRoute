...
provider order is config/env-driven and only exposed
...
automatic routing, or reset routing order from the dashboard.
...
**Current state**: Provider order is fixed in config; the dashboard can show status but not edit routing.

**Desired state**: An operator can reorder, enable/disable, and reset per-endpoint routing from the dashboard; the routing read paths honor persisted overrides and fall back to the built-in default order when no override exists.

## What Changes

- Persist a per-endpoint routing override (ordered provider list and per-provider enabled flag) for `search` and `fetch` chains via the existing settings store.
...
- Add dashboard UI in the existing search-tools config pane to reorder providers, toggle each provider on/off for automatic routing, and reset custom order, for both the search (main) and fetch (additional) chains.
...
- `openspec/specs/routing-config-ui/spec.md` - Adds operator-editable routing order and enable/disable config for both endpoints.
...
- `src/lib/search/searchChain.ts` / `src/app/api/v1/search/route.ts` - Read persisted search order/enabled override of the built-in order.
- `open-sse/handlers/webFetchChain.ts` / `src/app/api/v1/web/fetch/route.ts` - Read persisted web-fetch order/enabled override of the built-in order.
- `src/app/(dashboard)/dashboard/search-tools/components/SearchToolsConfigPane.tsx` - Reorder / toggle / reset controls for both chains.
...
- Provider order and automatic-routing enablement become inspectable and editable.
...
- Operator disables every provider: mitigate by validating that at least one compatible provider remains enabled.
